...
def heuristic(state, piece):
    kind = get_piece_kind(piece)
    dst = None
    if kind == 1:
        for i in range(5):
            if state[0][i]==0 and state[1][i]==0:
                dst = i
                break
        if dst != None:
            for i in range(5):
        ...
        for i in range(4):
            if state[1][i] + state[1][i+1] == 0:
                dst = i
                if piece[0][0] * piece[0][1] + piece[1][0] * piece[1][1] == 0:
                    return (i,1)
                else:
                    return (i,0)
        for i in range(5):
            if state[0][i] + state[1][i] == 0:
                dst = i
                if dst != 4:
                    if piece[0][0] * piece[1][0] != 0:
                        return (dst,0)
                    elif piece[0][1] * piece[1][1] != 0:
                        return (dst,2)
                    elif piece[0][0] * piece[0][1] != 0:
                        return (dst,1)
                    else:
                        return (dst,3)
                else:
        ...
        for i in range(4):
            if state[1][i] + state[1][i+1] == 0:
                if state[0][i] == 0 and state[0][i+1] == 1:
                    dst = (i,0)
                    break
                elif state[0][i] == 1 and state[0][i+1] == 0:
                    dst = (i,1)
                    break
        if dst == None:
            return None
        ...
        for i in range(4):
            if state[1][i] + state[1][i+1] + state[0][i] + state[0][i+1] == 0:
                return (i,0)
        for i in range(4):
            if state[1][i] + state[1][i+1] == 0:
                return (i,0)
...
class TetrisAI:
    ...
    def run(self, agent_host):
        ...
        avglvls = pickle.load(open("avglvls.pkl","rb"))
        avgclears = pickle.load(open("avgclears.pkl","rb"))
            ...
            for t in range(epochs):
                ...
                if t < T:
                    ...
                    if self.game.gameover == True:
                        ...
                        if game_overs == 5:
                            avglvls.append(numpy.mean(self.listGameLvl))
                            avgclears.append(numpy.mean(self.listClears))
                            print("Best attempt, gamesplayed: ", self.gamesplayed,
                                  " avglvls: ", avglvls[-1],
                                  " avgclears: ", avgclears[-1])
                            game_overs = 0
                            self.epsilon = 0
                        else:
                            self.epsilon = 0.3
                ...
                if (t+1)%1000 == 0:
                    ...
                    pickle.dump(avglvls,open("avglvls.pkl","wb"))
                    pickle.dump(avgclears,open("avgclears.pkl","wb"))
                    print("------------------Saving Qtable------------")
                    time.sleep(0.1)
    ...
    def choose_action(self, state, possible_actions):
        ...
        ######Heuristic######
        best_action = heuristic(state,self.game.piece)
        if best_action != None:
